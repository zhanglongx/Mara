...
8. 用户可以通过参数控制，指定数据的时间范围，例如从某年某月到某年某月。

9. 用户可以通过参数控制，指定输出的排序方式，例如按某个指标升序或降序排列输出结果。

11. 用户可以将上述数据导出为Excel xlsx文件。

12. 用户可以通过matplotlib对输出的指标进行可视化。当用户指定了多个指标时，可以生成多个子图。

13. 数据源接口尽量插件化，方便将来引入其他数据源。（现在可以没有）
...
- `--season INTEGER`：可选参数，指定查询的季度（1-4），如果指定了季度，则查询该季度的全部数据。默认为第四季度，即年报数据。

- `--latest`：可选参数，指定查询最新数据。

- `--aggregate TEXT`：可选参数，指定对季度数据进行的聚合操作，可以是"mean"（平均值）或"median"（中位值）。

- `--sort-by TEXT`：可选参数，指定排序的指标名称。
