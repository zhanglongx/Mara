...
EMBEDDED_API_FIELDS: dict[str, list[str]] = {
    "balancesheet": [
        "ts_code",
        "ann_date",
        "f_ann_date",
...
# Chinese labels from doc/ for reference only; not used at runtime.
EMBEDDED_API_FIELD_LABELS: dict[str, dict[str, str]] = {
    "balancesheet": {
        "ts_code": "TS股票代码",
        "ann_date": "公告日期",
        "f_ann_date": "实际公告日期",
...
def _build_api_specs() -> dict[str, ApiSpec]:
    ...
    for api_name, fields in EMBEDDED_API_FIELDS.items():
        api_specs[api_name] = ApiSpec(name=api_name, fields=list(fields))
    return api_specs
