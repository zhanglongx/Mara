...
class DataProcessor:
    ...
    def build_output_tables(
        ...
        for frequency, group_results in grouped.items():
            table_df: pd.DataFrame = self._merge_results(group_results, include_end_date)
            table_df = self._attach_basic_info(table_df)
            tables.append(OutputTable(frequency=frequency, data=table_df))

        return tables

    def _merge_results(
        self, results: list[IndicatorResult], include_end_date: bool
    ) -> pd.DataFrame:
