...
def export_excel(path: str, basic_info: pd.DataFrame, results: Iterable[IndicatorResult]) -> None:
    output_path: Path = Path(path).expanduser()
    sheets: list[ExcelSheet] = _build_sheets(basic_info, list(results))
    if not sheets:
        return
...
def _build_sheets(basic_info: pd.DataFrame, results: list[IndicatorResult]) -> list[ExcelSheet]:
    sheets: list[ExcelSheet] = []
    used_names: dict[str, int] = {}
    for result in results:
        df: pd.DataFrame = _attach_basic_info(basic_info, result.data)
        base_name: str = _sanitize_sheet_name(result.name)
        sheet_name: str = _dedupe_sheet_name(base_name, used_names)
...
def _dedupe_sheet_name(base: str, used: dict[str, int]) -> str:
    if base not in used:
        used[base] = 1
        return base
    used[base] += 1
