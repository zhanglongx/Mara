...
from mara.date_utils import DateRange, filter_quarter_dates, parse_yyyymmdd, to_yyyymmdd
...
def fetch_indicators(
    ...
    for indicator in indicator_list:
        api_name: str | None = registry.get_api(indicator)
        if api_name is None:
            custom_data: pd.DataFrame = _run_custom_indicator(
                registry, indicator, ts_code_list
            )
            results.append(IndicatorResult(name=indicator, frequency="custom", data=custom_data))
            continue
        grouped.setdefault(api_name, []).append(indicator)

    for api_name, api_indicators in grouped.items():
        api_df: pd.DataFrame = _fetch_api_data(
            client,
            api_name,
            api_indicators,
            ts_code_list,
        ...
        for indicator in api_indicators:
            ...
            results.append(IndicatorResult(name=indicator, frequency="quarterly", data=indicator_df))

    return results


def _run_custom_indicator(
    registry: IndicatorRegistry, indicator: str, ts_code_list: list[str]
) -> pd.DataFrame:
    custom_func: Callable[[pd.DataFrame], pd.Series] | None = registry.custom_indicators.get(
        indicator
    )
...
def _fetch_api_data(
    ...
    for ts_code in ts_codes:
        ...
        api_df: pd.DataFrame = client.query(api_name, **params)
        if api_df.empty:
            continue
    ...
    combined_df = _normalize_and_filter_range(combined_df, date_range)

    if single and api_name in SINGLE_QUARTER_APIS:
        combined_df = _to_single_quarter(combined_df, indicators)

    combined_df = _filter_by_season(combined_df, season)

    if latest:
        combined_df = _select_latest(combined_df)
    elif aggregate:
        combined_df = _aggregate_quarter(combined_df, indicators, aggregate)

    return combined_df


def _normalize_and_filter_range(data: pd.DataFrame, date_range: DateRange | None) -> pd.DataFrame:
    data = data.copy()
    end_dates: pd.Series = pd.to_datetime(data["end_date"], format="%Y%m%d", errors="coerce")
...
def _filter_by_season(data: pd.DataFrame, season: int) -> pd.DataFrame:
    if season not in (1, 2, 3, 4):
        return data
    data = data.copy()
    data["_end_date"] = pd.to_datetime(data["end_date"], format="%Y%m%d", errors="coerce")
    data = data[data["_end_date"].notna()]
    quarter_mask: list[bool] = filter_quarter_dates(data["_end_date"].dt.date.tolist(), season)
    data = data[pd.Series(quarter_mask, index=data.index)]
    data = data.drop(columns=["_end_date"])
    return data


def _to_single_quarter(data: pd.DataFrame, indicators: list[str]) -> pd.DataFrame:
    data = data.copy()
    data["_end_date"] = pd.to_datetime(data["end_date"], format="%Y%m%d", errors="coerce")
    data = data[data["_end_date"].notna()]
...
def _select_latest(data: pd.DataFrame) -> pd.DataFrame:
    data = data.copy()
    data["_end_date"] = pd.to_datetime(data["end_date"], format="%Y%m%d", errors="coerce")
    data = data[data["_end_date"].notna()]
...
def _aggregate_quarter(data: pd.DataFrame, indicators: list[str], agg: str) -> pd.DataFrame:
    data = data.copy()
    grouped: Any = data.groupby("ts_code", as_index=False)[indicators]
    if agg == "mean":
        agg_df: pd.DataFrame = grouped.mean(numeric_only=True)
    else:
        agg_df = grouped.median(numeric_only=True)
